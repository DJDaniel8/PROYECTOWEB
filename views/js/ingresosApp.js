...
document.getElementById("tablaIngresos").addEventListener("click", function(event){
...
function llenarTablaProductosIngreso(productos){
  ...
  productos.forEach(function(producto){
    ...
    cell4.textContent = "Q"+producto.precioCompra;
...
function requestProductosIngreso(id){
  const formData = new FormData();
    ...
    fetch(url+"Ingresos/productosOfIngresoById", {
      method: "POST",
      body: formData,
    })
    .then(response => {
    ...
    .then(data => {
      ...
      console.log(data.Valor);
      llenarTablaProductosIngreso(data.Valor);
    })
    .catch(error => {
